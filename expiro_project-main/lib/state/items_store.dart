...
class ItemsStore extends ChangeNotifier {
  ...
  int get expiredCount =>
      _items.where((e) => e.status == ItemStatus.expired).length;
  int get soonCount =>
      _items.where((e) => e.status == ItemStatus.soon).length;
  int get freshCount =>
      _items.where((e) => e.status == ItemStatus.fresh).length;
  ...
  Future<void> toggleFavorite(int id) async {
    ...
    _items[idx] = _items[idx].copyWith(
      isFavorite: !_items[idx].isFavorite,
    );
...
class ItemsScope extends InheritedNotifier<ItemsStore> {
  ...
  static ItemsStore of(BuildContext context) {
    ...
    assert(scope != null, 'ItemsScope not found');
